...
proc parsing_assign(vector<bra> const& in) {
	...
	for (; it != in.end(); ++it) { if (it->tk.typ == L"=") { break; } }
...
vector<proc> parsing_procs(vector<bra> const& in) {
	vector<bra> cur; vector<proc> out; bool curly = false;
	for (auto const& b : in) {
		if (b.tk.typ == L";") {
			if (!cur.empty()) { out.push_back(parsing_proc(cur)); } cur.clear();
		} else {
			if (b.tk.typ != L"else" && curly) {
				out.push_back(parsing_proc(cur)); cur.clear();
			} cur.push_back(b);
		} curly = b.typ == 2;
	}
