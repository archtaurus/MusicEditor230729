...
int Textbox::x_cursor_rel(App &app) const {
...
void Textbox::hdl_back(App& app) {
	...
	if (selected()) {
		...
		x_str_rel = min<double>(x_str_rel, w_edit_margin);
		auto a = str.begin();
...
void Textbox::clamp_cs_not_selecting(App& app) {
	if (x_cursor_rel(app) > max_x_cursor_rel()) {
		x_str_rel -= x_cursor_rel(app) - max_x_cursor_rel();
	} else if (x_cursor_rel(app) < w_edit_margin) {
		x_str_rel += w_edit_margin - x_cursor_rel(app);
	}
}
...
void Textbox::render_main(App& app) {
	...
	draw_str(scr, dscr, dep, left(), c_txt, ft, tl_txt, x_cur, 0, vp_show_txt());
	draw_str(scr, dscr, dep, mid(), c_selected, ft, tl_txt, x_cur, 0, vp_show_txt());
	draw_str(scr, dscr, dep, right(), c_txt, ft, tl_txt, x_cur, 0, vp_show_txt());
}
void Textbox::render_cursor(App& app) {
	bool show_cursor = edit &&
		(sin(dur * twinkle_mtp) > 0 || cursor_change);
	if (show_cursor) {
		int x_cs = tl.x + x_cursor_rel(app) - w_cursor / 2;
		int y_cs = tl.y + (h - h_cursor) / 2;
