...
void FloatPanel::deal_caption(App &app) {
	...
	if (dragged_bar) {
		...
		tmp.y = clmp(tmp.y, 0, scr.h - h_bar); set_tl(tmp);
		dragged_bar = msd[0];
...
void FloatPanel::render_main(App &app) {
	...
	draw_px_rect_framed_raw(scr, tl, w, h_bar, vp, c_bar, c_frame);
...
void FloatPanel::render_close(App& app) {
	...
	draw_px_seg(scr, dscr, mark_ct - mark_arm, mark_ct + mark_arm, dep, vp, c_mark);
...
void FPnLan::Update(App &app) {
	txt = loc(id); FloatPanel::Update(app);
}
