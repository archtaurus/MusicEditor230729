...
void ScrollbarY::render(App &app) {
	draw_px_rect_framed_raw(scr, tl, w, h, vp, c_bar, c_frame);
	if (full) {
		dcol const& c =
			!enabled ? c_invalid :
			dragged ? c_dragged :
			hovered_node ? c_hovered : c_normal;
		draw_px_rect_framed_raw(scr, tl + dvec(0, ny_rel), w, nh, vp, c, c_frame);
	}
}
...
void ScrollbarY::Update(App& app) {
	...
	if (full) {
		...
		if (dragged) {
			...
			ny_rel = clmp(ny_rel, 0, h - nh);
			dragged = full && enabled && hovered_node && msc(0);
		}
	} else { dragged = false; } render(app);
}
void ScrollbarY::Discard(App &app) {
	rmv; dragged = hovered = hovered_node = full = false;
}
