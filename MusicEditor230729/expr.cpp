...
expr::expr(wstring const& fun, vector<expr> const& pars) :
	fun(fun), pars(pars) { typ = L"call"; }
expr expr::mkvar(wstring const& var) {
	auto ex = expr(L"var");
	ex.var = var; return ex;
}
...
bool parsing_neg(vector<pre_expr> const& in, expr& out) {
	rep(i, 0, in.size()) {
		...
		if (i == 0 || in[i - 1].istk) {
			...
			tmp.push_back(pre_expr(expr(s + s, { in[i + 1].ex })));
...
bool parsing_bin
(vector<pre_expr> const& in, expr& out, vector<wstring> const& mark) {
	rep(i, 0, in.size()) {
		auto const& pre = in[i];
		if (!pre.istk) { continue; }
		auto const& s = pre.tk.typ;
		if (find(mark.begin(), mark.end(), s) != mark.end()) {
			...
			tmp.push_back(pre_expr(expr(s, { in[i - 1].ex, in[i + 1].ex })));
			rep(j, i + 2, in.size()) { tmp.push_back(in[j]); }
			out = parsing_pre_expr(tmp); return true;
		}
	} return false;
...
expr parsing_pre_expr(vector<pre_expr> const& in) {
	...
	if (in.size() == 1) {
		if (in[0].istk) { return expr(L"error"); } return in[0].ex;
	}
	expr out;
...
expr parsing_call(vector<bra> const& in) {
	expr ex(L"call"); ex.fun = in[0].tk.var;
	if (!in[1].bras.empty()) {
		vector<bra> tmp;
		for (auto const& b : in[1].bras) {
			if (b.tk.typ == L",") {
				ex.pars.push_back(parsing_expr(tmp)); tmp.clear();
...
expr parsing_bins(vector<bra> const& in) {
	...
	for (auto const& b : in) {
		if (is_bin_operator(b.tk)) {
			if (!tmp.empty()) {
				pres.push_back(pre_expr(parsing_expr(tmp))); tmp.clear();
			} pres.push_back(pre_expr(b.tk));
		} else { tmp.push_back(b); }
	}
	if (!tmp.empty()) { pres.push_back(pre_expr(parsing_expr(tmp))); }
...
expr parsing_vec(vector<bra> const& in) {
	...
	for (auto const& b : in[0].bras) {
		if (b.tk.typ == L",") {
			ex.vec.push_back(parsing_expr(tmp)); tmp.clear(); continue;
		} tmp.push_back(b);
	}
...
expr parsing_expr(vector<bra> const& in) {
	...
	if (in.size() == 1) {
		if (b.typ == 0) {
			if (b.tk.typ == L"var") { return expr::mkvar(b.tk.var); }
