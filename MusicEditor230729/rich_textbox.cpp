...
dvec RichTextbox::p_cursor_rel(App &app) const {
...
void RichTextbox::update_x_cursor_rel_hist(App& app) {
...
void RichTextbox::clamp_cs_not_selecting(App& app) {
	if (p_cursor_rel(app).x > max_x_cursor_rel()) {
		tl_str_rel.x -= p_cursor_rel(app).x - max_x_cursor_rel();
...
void RichTextbox::render_main(App& app) {
	...
	draw_str(scr, dscr, dep, left(), c_txt, ft, tl_txt, x_cur, w_txt, vp_show_txt());
	draw_str(scr, dscr, dep, mid(), c_selected, ft, tl_txt, x_cur, w_txt, vp_show_txt());
	draw_str(scr, dscr, dep, right(), c_txt, ft, tl_txt, x_cur, w_txt, vp_show_txt());
}
void RichTextbox::render_cursor(App& app) {
	bool show_cursor = edit &&
		(sin(dur * twinkle_mtp) > 0 || cursor_change);
	if (show_cursor) {
		dvec tl_cs = tl + p_cursor_rel(app) + dvec(-w_cursor, ft.h - h_cursor) / 2;
