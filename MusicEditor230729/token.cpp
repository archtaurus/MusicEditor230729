...
token token::mkvar(wstring const& var) {
	token tk(L"var");
	tk.var = var; return tk;
}
...
token get_tk_num(wstring const& in, int& id) {
	...
	while (ok && ++id != in.size()) {
		auto c = in[id];
		if (c == L'.') {
			if (dot) { return token(L"error"); } dot = true;
		}
		ok = iswdigit(c) || c == L'.';
		if (ok) { tmp += c; }
	}
...
token get_tk_str(wstring const& in, int& id) {
	...
	while (ok && ++id != in.size()) {
		...
		if (reserved) {
			if (c == L'n') { c = L'\n'; }
			reserved = false; ok = true;
		} else { reserved = c == L'\\'; ok = c != L'\"'; }
		if (ok && !reserved) { tmp += c; }
	} ++id; 
...
vector<token> deal_sgn(vector<token> const& in) {
	...
	for (auto const& tk : in) {
		...
		if (add) {
			...
			out.push_back(sgn == 1 ? token(L"+") : token(L"-"));
		} out.push_back(tk);
	}
	if (add) { out.push_back(sgn == 1 ? token(L"+") : token(L"-")); } 
	return out;
}
